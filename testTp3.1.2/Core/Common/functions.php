...
function U($url='',$vars='',$suffix=true,$redirect=false,$domain=false) {
    ...
    if(isset($host)) {
        ...
        if(C('APP_SUB_DOMAIN_DEPLOY') ) { // 开启子域名部署
			...
			$domain = $domain=='localhost'?'localhost':'www.'.$_SERVER['HTTP_HOST'];
			}else{
            ...
            foreach ($vars as $var => $val){			
				if('' !== trim($val)){
					...
					if(C('APP_SUB_DOMAIN_DEPLOY') ) { // 开启子域名部署
						foreach (C('APP_SUB_DOMAIN_RULES') as $key => $rule) {						
							$city = strstr($rule[1],'=');
							$city=str_replace($city,'',$rule[1]);							
							$rule[0]= str_replace('/',$depr,strtolower($rule[0]));	
							//dump('/'.$rule[0]);
							if(false !== strpos($key,'*') && 0=== strpos($url,'/'.$rule[0])) {								
								if($city ===trim($var)){
									$domain = $val.strstr($domain,'.'); // 生成对应子域名									
									//$url    = session('_curUrl_')?session('_curUrl_'):Cookie::get('_curUrl_'); 										
							...
							}elseif(false === strpos($key,'*') && 0=== strpos($url,'/'.$rule[0])) {																
	...
	/**
     * SSQ URL子域名部署以及路由规则配置 START
     * @author lingyun
     */
		//根据子域名设置，分页的处理
		if(C('APP_SUB_DOMAIN_DEPLOY') ) { // 开启子域名部署
			foreach (C('APP_SUB_DOMAIN_RULES') as $key => $rule) {			
				$rule[0]=str_replace('/',$depr,$rule[0]);				
				if(false !== strpos($key,'*') && 0=== strpos($url,strtolower('/'.$rule[0]))) {						
					$rule[1] =strtolower(str_replace('=',$depr,$rule[1]));
					$rule[1] =strtolower(str_replace('*',SUB_DOMAIN,$rule[1]));			
					if(strstr($url,$depr.$rule[1])){
					$url=str_replace($depr.$rule[1],'',$url);						
					}						
				}							
			}			
		}
    ...
    foreach (C('URL_ROUTE_RULES') as $zhaolg => $zlig){//遍历路由   
		$zlig=	strtolower(str_replace('/',$depr,$zlig));	
        if(strstr($url,$zlig)){//从网址中找路由规则	
        $lg = strstr($zhaolg,'/:');//分析规则 带参数?	
		$url = str_replace($zlig,trim(str_replace($lg,'',$zhaolg)),$url);//url替换路由规则参数 
        if($lg){//如果规则带参数 			
                $tempzlg = str_replace('\d','',trim($lg));//取参数  
				$tempzlg=explode('/:',$tempzlg);
				unset($tempzlg[0]);							
                if(is_array($tempzlg)){
					foreach($tempzlg as $value){
						if(strstr($url,$depr.$value.$depr)){
						$url=str_replace($depr.$value.$depr,$depr,$url);						
						}	
					}
                }    
            }else{//分析规则  不带参数
